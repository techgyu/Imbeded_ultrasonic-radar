...
# 파일명: Servo_90_angle.py
# ------
# 서보 모터를 90도 위치로 제어하는 라즈베리파이 예제 코드
# - RPi.GPIO 라이브러리 사용
...
# 90도 위치로 서보 모터 제어 및 예외 처리
try:
    while True:
        duty = 7.5  # 90도에 해당하는 duty cycle (서보마다 다를 수 있음)
        pwm.ChangeDutyCycle(duty)  # duty cycle 변경 (90도 위치)
        time.sleep(2)              # 2초 대기
